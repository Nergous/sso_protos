// Code generated by protoc-gen-go-grpc. DO NOT EDIT.
// versions:
// - protoc-gen-go-grpc v1.6.1
// - protoc             (unknown)
// source: sso/auth/v2/app.proto

package ssov2

import (
	context "context"
	grpc "google.golang.org/grpc"
	codes "google.golang.org/grpc/codes"
	status "google.golang.org/grpc/status"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
)

// This is a compile-time assertion to ensure that this generated file
// is compatible with the grpc package it is being compiled against.
// Requires gRPC-Go v1.64.0 or later.
const _ = grpc.SupportPackageIsVersion9

const (
	App_GetApp_FullMethodName            = "/sso.auth.v2.App/GetApp"
	App_GetAllApps_FullMethodName        = "/sso.auth.v2.App/GetAllApps"
	App_CreateApp_FullMethodName         = "/sso.auth.v2.App/CreateApp"
	App_UpdateApp_FullMethodName         = "/sso.auth.v2.App/UpdateApp"
	App_DeleteApp_FullMethodName         = "/sso.auth.v2.App/DeleteApp"
	App_ChangeStatusApp_FullMethodName   = "/sso.auth.v2.App/ChangeStatusApp"
	App_IsAdmin_FullMethodName           = "/sso.auth.v2.App/IsAdmin"
	App_AddAdmin_FullMethodName          = "/sso.auth.v2.App/AddAdmin"
	App_RemoveAdmin_FullMethodName       = "/sso.auth.v2.App/RemoveAdmin"
	App_GetAllUsersForApp_FullMethodName = "/sso.auth.v2.App/GetAllUsersForApp"
)

// AppClient is the client API for App service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// App manages the third-party applications registered with the SSO service
// and the set of administrators for each of them.
//
// Error handling: see errors.proto.
type AppClient interface {
	// GetApp returns a single app by id.
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	GetApp(ctx context.Context, in *GetAppRequest, opts ...grpc.CallOption) (*GetAppResponse, error)
	// GetAllApps returns a page of apps. Pagination follows AIP-158 (see
	// GetAllUsers).
	GetAllApps(ctx context.Context, in *GetAllAppsRequest, opts ...grpc.CallOption) (*GetAllAppsResponse, error)
	// CreateApp registers a new app and returns it.
	// Possible reasons: APP_ALREADY_EXISTS, VALIDATION_FAILED.
	CreateApp(ctx context.Context, in *CreateAppRequest, opts ...grpc.CallOption) (*CreateAppResponse, error)
	// UpdateApp updates the mutable fields of the app with the given id.
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	UpdateApp(ctx context.Context, in *UpdateAppRequest, opts ...grpc.CallOption) (*UpdateAppResponse, error)
	// DeleteApp removes the app with the given id. Idempotent.
	// Possible reasons: VALIDATION_FAILED.
	DeleteApp(ctx context.Context, in *DeleteAppRequest, opts ...grpc.CallOption) (*emptypb.Empty, error)
	// ChangeStatusApp toggles the enabled/disabled flag of an app.
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	ChangeStatusApp(ctx context.Context, in *ChangeStatusAppRequest, opts ...grpc.CallOption) (*emptypb.Empty, error)
	// IsAdmin reports whether the given user is an administrator of the
	// given app.
	// Possible reasons: USER_NOT_FOUND, APP_NOT_FOUND, VALIDATION_FAILED.
	IsAdmin(ctx context.Context, in *IsAdminRequest, opts ...grpc.CallOption) (*IsAdminResponse, error)
	// AddAdmin grants administrator rights on the app to the user. Idempotent.
	// Possible reasons: USER_NOT_FOUND, APP_NOT_FOUND, PERMISSION_DENIED,
	// VALIDATION_FAILED.
	AddAdmin(ctx context.Context, in *AddAdminRequest, opts ...grpc.CallOption) (*emptypb.Empty, error)
	// RemoveAdmin revokes administrator rights on the app from the user.
	// Idempotent.
	// Possible reasons: USER_NOT_FOUND, APP_NOT_FOUND, PERMISSION_DENIED,
	// VALIDATION_FAILED.
	RemoveAdmin(ctx context.Context, in *RemoveAdminRequest, opts ...grpc.CallOption) (*emptypb.Empty, error)
	// GetAllUsersForApp returns a page of users associated with the given
	// app, annotating each with the is_admin flag. Pagination follows
	// AIP-158 (see GetAllUsers).
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	GetAllUsersForApp(ctx context.Context, in *GetAllUsersForAppRequest, opts ...grpc.CallOption) (*GetAllUsersForAppResponse, error)
}

type appClient struct {
	cc grpc.ClientConnInterface
}

func NewAppClient(cc grpc.ClientConnInterface) AppClient {
	return &appClient{cc}
}

func (c *appClient) GetApp(ctx context.Context, in *GetAppRequest, opts ...grpc.CallOption) (*GetAppResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GetAppResponse)
	err := c.cc.Invoke(ctx, App_GetApp_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) GetAllApps(ctx context.Context, in *GetAllAppsRequest, opts ...grpc.CallOption) (*GetAllAppsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GetAllAppsResponse)
	err := c.cc.Invoke(ctx, App_GetAllApps_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) CreateApp(ctx context.Context, in *CreateAppRequest, opts ...grpc.CallOption) (*CreateAppResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(CreateAppResponse)
	err := c.cc.Invoke(ctx, App_CreateApp_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) UpdateApp(ctx context.Context, in *UpdateAppRequest, opts ...grpc.CallOption) (*UpdateAppResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(UpdateAppResponse)
	err := c.cc.Invoke(ctx, App_UpdateApp_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) DeleteApp(ctx context.Context, in *DeleteAppRequest, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, App_DeleteApp_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) ChangeStatusApp(ctx context.Context, in *ChangeStatusAppRequest, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, App_ChangeStatusApp_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) IsAdmin(ctx context.Context, in *IsAdminRequest, opts ...grpc.CallOption) (*IsAdminResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(IsAdminResponse)
	err := c.cc.Invoke(ctx, App_IsAdmin_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) AddAdmin(ctx context.Context, in *AddAdminRequest, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, App_AddAdmin_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) RemoveAdmin(ctx context.Context, in *RemoveAdminRequest, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, App_RemoveAdmin_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *appClient) GetAllUsersForApp(ctx context.Context, in *GetAllUsersForAppRequest, opts ...grpc.CallOption) (*GetAllUsersForAppResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GetAllUsersForAppResponse)
	err := c.cc.Invoke(ctx, App_GetAllUsersForApp_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// AppServer is the server API for App service.
// All implementations must embed UnimplementedAppServer
// for forward compatibility.
//
// App manages the third-party applications registered with the SSO service
// and the set of administrators for each of them.
//
// Error handling: see errors.proto.
type AppServer interface {
	// GetApp returns a single app by id.
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	GetApp(context.Context, *GetAppRequest) (*GetAppResponse, error)
	// GetAllApps returns a page of apps. Pagination follows AIP-158 (see
	// GetAllUsers).
	GetAllApps(context.Context, *GetAllAppsRequest) (*GetAllAppsResponse, error)
	// CreateApp registers a new app and returns it.
	// Possible reasons: APP_ALREADY_EXISTS, VALIDATION_FAILED.
	CreateApp(context.Context, *CreateAppRequest) (*CreateAppResponse, error)
	// UpdateApp updates the mutable fields of the app with the given id.
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	UpdateApp(context.Context, *UpdateAppRequest) (*UpdateAppResponse, error)
	// DeleteApp removes the app with the given id. Idempotent.
	// Possible reasons: VALIDATION_FAILED.
	DeleteApp(context.Context, *DeleteAppRequest) (*emptypb.Empty, error)
	// ChangeStatusApp toggles the enabled/disabled flag of an app.
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	ChangeStatusApp(context.Context, *ChangeStatusAppRequest) (*emptypb.Empty, error)
	// IsAdmin reports whether the given user is an administrator of the
	// given app.
	// Possible reasons: USER_NOT_FOUND, APP_NOT_FOUND, VALIDATION_FAILED.
	IsAdmin(context.Context, *IsAdminRequest) (*IsAdminResponse, error)
	// AddAdmin grants administrator rights on the app to the user. Idempotent.
	// Possible reasons: USER_NOT_FOUND, APP_NOT_FOUND, PERMISSION_DENIED,
	// VALIDATION_FAILED.
	AddAdmin(context.Context, *AddAdminRequest) (*emptypb.Empty, error)
	// RemoveAdmin revokes administrator rights on the app from the user.
	// Idempotent.
	// Possible reasons: USER_NOT_FOUND, APP_NOT_FOUND, PERMISSION_DENIED,
	// VALIDATION_FAILED.
	RemoveAdmin(context.Context, *RemoveAdminRequest) (*emptypb.Empty, error)
	// GetAllUsersForApp returns a page of users associated with the given
	// app, annotating each with the is_admin flag. Pagination follows
	// AIP-158 (see GetAllUsers).
	// Possible reasons: APP_NOT_FOUND, VALIDATION_FAILED.
	GetAllUsersForApp(context.Context, *GetAllUsersForAppRequest) (*GetAllUsersForAppResponse, error)
	mustEmbedUnimplementedAppServer()
}

// UnimplementedAppServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedAppServer struct{}

func (UnimplementedAppServer) GetApp(context.Context, *GetAppRequest) (*GetAppResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetApp not implemented")
}
func (UnimplementedAppServer) GetAllApps(context.Context, *GetAllAppsRequest) (*GetAllAppsResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetAllApps not implemented")
}
func (UnimplementedAppServer) CreateApp(context.Context, *CreateAppRequest) (*CreateAppResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method CreateApp not implemented")
}
func (UnimplementedAppServer) UpdateApp(context.Context, *UpdateAppRequest) (*UpdateAppResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method UpdateApp not implemented")
}
func (UnimplementedAppServer) DeleteApp(context.Context, *DeleteAppRequest) (*emptypb.Empty, error) {
	return nil, status.Error(codes.Unimplemented, "method DeleteApp not implemented")
}
func (UnimplementedAppServer) ChangeStatusApp(context.Context, *ChangeStatusAppRequest) (*emptypb.Empty, error) {
	return nil, status.Error(codes.Unimplemented, "method ChangeStatusApp not implemented")
}
func (UnimplementedAppServer) IsAdmin(context.Context, *IsAdminRequest) (*IsAdminResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method IsAdmin not implemented")
}
func (UnimplementedAppServer) AddAdmin(context.Context, *AddAdminRequest) (*emptypb.Empty, error) {
	return nil, status.Error(codes.Unimplemented, "method AddAdmin not implemented")
}
func (UnimplementedAppServer) RemoveAdmin(context.Context, *RemoveAdminRequest) (*emptypb.Empty, error) {
	return nil, status.Error(codes.Unimplemented, "method RemoveAdmin not implemented")
}
func (UnimplementedAppServer) GetAllUsersForApp(context.Context, *GetAllUsersForAppRequest) (*GetAllUsersForAppResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method GetAllUsersForApp not implemented")
}
func (UnimplementedAppServer) mustEmbedUnimplementedAppServer() {}
func (UnimplementedAppServer) testEmbeddedByValue()             {}

// UnsafeAppServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to AppServer will
// result in compilation errors.
type UnsafeAppServer interface {
	mustEmbedUnimplementedAppServer()
}

func RegisterAppServer(s grpc.ServiceRegistrar, srv AppServer) {
	// If the following call panics, it indicates UnimplementedAppServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
		t.testEmbeddedByValue()
	}
	s.RegisterService(&App_ServiceDesc, srv)
}

func _App_GetApp_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetAppRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).GetApp(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_GetApp_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).GetApp(ctx, req.(*GetAppRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_GetAllApps_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetAllAppsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).GetAllApps(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_GetAllApps_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).GetAllApps(ctx, req.(*GetAllAppsRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_CreateApp_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(CreateAppRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).CreateApp(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_CreateApp_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).CreateApp(ctx, req.(*CreateAppRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_UpdateApp_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(UpdateAppRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).UpdateApp(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_UpdateApp_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).UpdateApp(ctx, req.(*UpdateAppRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_DeleteApp_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(DeleteAppRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).DeleteApp(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_DeleteApp_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).DeleteApp(ctx, req.(*DeleteAppRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_ChangeStatusApp_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ChangeStatusAppRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).ChangeStatusApp(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_ChangeStatusApp_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).ChangeStatusApp(ctx, req.(*ChangeStatusAppRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_IsAdmin_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(IsAdminRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).IsAdmin(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_IsAdmin_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).IsAdmin(ctx, req.(*IsAdminRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_AddAdmin_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(AddAdminRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).AddAdmin(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_AddAdmin_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).AddAdmin(ctx, req.(*AddAdminRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_RemoveAdmin_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(RemoveAdminRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).RemoveAdmin(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_RemoveAdmin_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).RemoveAdmin(ctx, req.(*RemoveAdminRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _App_GetAllUsersForApp_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetAllUsersForAppRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AppServer).GetAllUsersForApp(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: App_GetAllUsersForApp_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AppServer).GetAllUsersForApp(ctx, req.(*GetAllUsersForAppRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// App_ServiceDesc is the grpc.ServiceDesc for App service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var App_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "sso.auth.v2.App",
	HandlerType: (*AppServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "GetApp",
			Handler:    _App_GetApp_Handler,
		},
		{
			MethodName: "GetAllApps",
			Handler:    _App_GetAllApps_Handler,
		},
		{
			MethodName: "CreateApp",
			Handler:    _App_CreateApp_Handler,
		},
		{
			MethodName: "UpdateApp",
			Handler:    _App_UpdateApp_Handler,
		},
		{
			MethodName: "DeleteApp",
			Handler:    _App_DeleteApp_Handler,
		},
		{
			MethodName: "ChangeStatusApp",
			Handler:    _App_ChangeStatusApp_Handler,
		},
		{
			MethodName: "IsAdmin",
			Handler:    _App_IsAdmin_Handler,
		},
		{
			MethodName: "AddAdmin",
			Handler:    _App_AddAdmin_Handler,
		},
		{
			MethodName: "RemoveAdmin",
			Handler:    _App_RemoveAdmin_Handler,
		},
		{
			MethodName: "GetAllUsersForApp",
			Handler:    _App_GetAllUsersForApp_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "sso/auth/v2/app.proto",
}
